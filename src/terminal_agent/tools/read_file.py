...
def _read_file_sync(file_path: Path, start_line: Optional[int], end_line: Optional[int]) -> ToolResult:
    ...
    if not file_path.is_file():
        return ToolResult(
            output=f"Error: Path '{file_path}' is not a file.", 
            is_error=True
        )
    ...
    try:
        ...
        return ToolResult(
            output=f"Error: File '{file_path}' appears to be binary or has an unsupported encoding.",
            is_error=True
        )
    except PermissionError:
        return ToolResult(
            output=f"Error: Permission denied when accessing '{file_path}'.",
            is_error=True
        )
